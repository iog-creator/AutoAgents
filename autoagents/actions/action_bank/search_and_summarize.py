...
class SearchAndSummarize(Action):
    ...
    async def run(self, context: list[Message], system_text=SEARCH_AND_SUMMARIZE_SYSTEM) -> str:
        no_serpapi = (
            ...
            or self.config.serpapi_api_key == 'YOUR_API_KEY'
        )
        no_serper = (
            not self.config.serper_api_key
            or self.config.serper_api_key == 'YOUR_API_KEY'
        )
        no_google = (
            not self.config.google_api_key
            or self.config.google_api_key == 'YOUR_API_KEY'
        )
